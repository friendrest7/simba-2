...
export const Route = createFileRoute("/category/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      ...
      { name: "description", content: `Shop ${loaderData?.cat.name ?? "products"} at Simba Supermarket Rwanda.` },
    ],
  }),
...
function CategoryPage() {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="relative mb-8 overflow-hidden rounded-3xl border border-border p-8 shadow-sm md:p-12" style={{ background: `linear-gradient(135deg, color-mix(in oklab, ${cat.color} 22%, var(--card)), var(--card))` }}>
        <div className="absolute -right-10 -top-10 h-40 w-40 rounded-full opacity-30 blur-3xl" style={{ background: cat.color }} />
        <div className="text-6xl">{cat.emoji}</div>
        <h1 className="mt-3 text-4xl font-black tracking-tight">{categoryLabel(cat.name, t)}</h1>
        <p className="mt-1 text-sm text-muted-foreground">{allProducts.length} {t("section.items")}</p>
      </div>
      ...
      <div className="sticky top-16 z-30 mb-8 flex flex-col gap-4 bg-background/90 py-4 backdrop-blur md:flex-row">
        <div className="relative flex-1">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t("ui.searchInCategory")} className="h-12 rounded-full border-input bg-muted/50 pl-10 text-base focus-visible:ring-primary/50" />
        </div>
        ...
        <div className="hidden items-center gap-3 md:flex">
          <select value={sort} onChange={(e) => setSort(e.target.value)} className="h-12 rounded-full border border-input bg-background px-4 text-base font-medium text-foreground shadow-sm focus:border-primary/50 focus:ring-primary/50">
      ...
      <div className={`fixed inset-0 z-50 bg-background/95 backdrop-blur transition-transform duration-300 ease-in-out md:hidden ${showFilters ? "translate-y-0" : "translate-y-full"}`}>
        ...
        <div className="p-4">
          <div className="mb-6">
            ...
            <select value={sort} onChange={(e) => setSort(e.target.value)} className="h-11 w-full rounded-xl border border-input bg-background px-4 text-base font-medium text-foreground shadow-sm focus:border-primary/50 focus:ring-primary/50">
              <option value="popular">{t("sort.popular")}</option>
              <option value="priceAsc">{t("sort.priceAsc")}</option>
              <option value="priceDesc">{t("sort.priceDesc")}</option>
              <option value="name">{t("sort.name")}</option>
      ...
      {filteredProducts.length === 0 ? (
        <div className="py-20 text-center">
          ...
          <button onClick={clearFilters} className="mt-4 inline-block font-semibold text-primary hover:underline">
