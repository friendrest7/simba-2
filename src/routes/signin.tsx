...
function SignInPage() {
  ...
  const submitGoogle = () => {
    // Demo Google sign-in (no real OAuth wired). Treat as a quick demo identity.
  ...
  return (
    <div className="relative mx-auto flex min-h-[calc(100vh-8rem)] max-w-md items-center px-4 py-10">
      <div className="absolute inset-0 -z-10 overflow-hidden">
        <div className="absolute top-1/4 left-1/2 h-72 w-72 -translate-x-1/2 rounded-full bg-primary/20 blur-3xl" />
      </div>

      <div className="w-full rounded-3xl border border-border bg-card/80 p-8 backdrop-blur-xl shadow-2xl shadow-primary/10">
        ...
        <Button
          ...
          variant="outline"
          size="lg"
          className="mt-6 w-full rounded-full gap-2 h-11 font-semibold"
        >
        ...
        <Tabs defaultValue="email">
          <TabsList className="grid w-full grid-cols-2 rounded-full">
            <TabsTrigger value="email" className="rounded-full gap-1.5">
              <Mail className="h-3.5 w-3.5" /> {t("signin.tab.email")}
            </TabsTrigger>
            <TabsTrigger value="phone" className="rounded-full gap-1.5">
          ...
          <TabsContent value="email">
            <form onSubmit={submitEmail} className="mt-4 space-y-4">
              ...
              <div>
                ...
                <Input id="password" type="password" required placeholder="••••••••" className="mt-1.5 h-11 rounded-xl" />
              </div>
              ...
              <form onSubmit={sendOtp} className="mt-4 space-y-4">
                <div>
                  ...
                  <div className="mt-1.5 flex gap-2">
                    <span className="inline-flex h-11 items-center rounded-xl border border-border bg-secondary px-3 text-sm font-semibold">
                      +250
                    </span>
                    <Input
                      id="phone"
              ...
              <form onSubmit={verifyOtp} className="mt-4 space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    placeholder="123456"
                    className="mt-1.5 h-11 rounded-xl tracking-[0.5em] text-center font-mono"
                  />
                  <p className="mt-1.5 text-[11px] text-muted-foreground">
